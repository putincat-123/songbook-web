(() => {
  ...
  const THEME={
    outline:'#6A4532', outline2:'#875B43',
    wall:'#F7F0E6', wallLine:'#E7D7C5', baseboard:'#A97654',
    floorA:'#C89D73', floorB:'#D2AA82', floorLine:'#B2835B',
    glassA:'#BFE5F2', glassB:'#8FC8DD', window:'#7A5440',
    honey:'#E7BC63', honeyDark:'#C89142', honeyLight:'#F6E0A8', coral:'#D96B63',
    walnut:'#7A503B', walnutDark:'#6A4532', gold:'#D6AF63', incense:'#B84A46',
    wood:'#8E5736', woodHi:'#B7774E', woodDark:'#6E4128', mallet:'#6B4631', malletHead:'#B4845B',
    beads:'#8A5A3A', beadsHi:'#A9744B', bed:'#EED6A5', bedShade:'#D8B985', bedLeg:'#A67B5A'
  };
  ...
  .px-wrap{max-width:620px;margin:10px auto;font-family:"Courier New",monospace}.px-shell{background:#5B4136;border:4px solid #4A342C;box-shadow:0 8px 0 #3C2923,0 16px 30px rgba(74,52,44,.18);border-radius:12px;overflow:hidden}.px-hud{display:flex;gap:6px;align-items:center;justify-content:center;flex-wrap:wrap;background:linear-gradient(#8A6554,#755142);color:#FFF8EC;padding:9px 8px;border-bottom:4px solid #563B31;font-weight:900}.px-chip{background:#A27962;border:2px solid #563B31;padding:5px 7px;font-size:12px}.px-room{position:relative;background:${THEME.floorA}}.px-room canvas{display:block;width:100%;height:auto;image-rendering:pixelated;touch-action:manipulation}.px-bubble{position:absolute;left:12%;right:12%;top:4.5%;background:#FFF9EF;border:2px solid #875B43;box-shadow:3px 3px 0 rgba(106,69,50,.18);padding:6px 9px;text-align:center;font:900 12px/1.35 -apple-system,BlinkMacSystemFont,"PingFang SC",sans-serif;color:#5D4235;pointer-events:none;transition:opacity .25s ease,transform .25s ease}.px-tip{background:#6A4B3F;color:#F4E8D6;padding:5px 8px;text-align:center;font:800 10px/1.4 -apple-system,BlinkMacSystemFont,"PingFang SC",sans-serif;opacity:.9}.px-actions{display:grid;grid-template-columns:repeat(4,1fr);gap:7px;padding:8px;background:#765447;border-top:4px solid #563B31}.px-btn{border:2px solid #6A4532;background:linear-gradient(#FFF4DA,#EED5A5);color:#4D3428;box-shadow:0 3px 0 #6A4532;padding:10px 4px;font-weight:950;font-size:12px}.px-manage{display:flex;gap:7px;padding:7px 8px 9px;background:#65483D;justify-content:center;flex-wrap:wrap}.px-small{border:2px solid #6A4532;background:#DEC7AE;padding:6px 9px;font-weight:900;font-size:11px;color:#4D3428}.px-sound.on{background:#DCEBD5}.px-adopt{background:#F7E8CB;border:4px solid #6A4532;padding:14px}.px-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:12px 0}.px-choice{border:3px solid #875B43;background:#FFF3D6;padding:10px 5px;text-align:center;font-weight:900}.px-choice.sel{background:#DFF1F7;border-color:#5F91A6}.px-icon{width:70px;height:60px;margin:0 auto 5px;image-rendering:pixelated}.px-name{width:100%;border:3px solid #875B43;background:#FFFCF6;padding:11px;font-size:16px;font-weight:900}.px-adoptgo{width:100%;margin-top:8px;border:3px solid #476D7D;background:#77B7CF;color:#fff;padding:11px;font-weight:950}.px-float{position:absolute;z-index:9;pointer-events:none;color:#9B5B2E;font:900 12px/1 -apple-system,BlinkMacSystemFont,"PingFang SC",sans-serif;text-shadow:0 1px 0 #FFE8BD;animation:pxFloat .72s ease-out forwards;white-space:nowrap}@keyframes pxFloat{0%{opacity:0;transform:translate(-50%,4px) scale(.92)}16%{opacity:1}100%{opacity:0;transform:translate(-50%,-30px) scale(1.05)}}@media(max-width:620px){.px-wrap{margin:8px -2px}.px-actions{grid-template-columns:repeat(2,1fr)}.px-grid{grid-template-columns:repeat(2,1fr)}}`;
  ...
  const HIT={blind:{x:150,y:190,r:31},incense:{x:251,y:187,r:25},wood:{x:82,y:248,r:34},beads:{x:250,y:299,r:28},pray:{x:75,y:360,r:31}};
  ...
  function drawRoom(g){
    ...
    outline(g,174,84,74,16,'#F3E2BE',THEME.outline2);
    ...
    outline(g,219,201,65,11,THEME.walnut,THEME.outline);rect(g,225,212,7,37,THEME.walnutDark);rect(g,271,212,7,37,THEME.walnutDark);outline(g,238,181,27,13,THEME.gold,THEME.outline2);rect(g,246,158,3,24,THEME.incense);rect(g,254,160,3,22,THEME.incense);

    outline(g,48,222,104,74,'#C39A76',THEME.outline2);rect(g,55,229,90,60,'#D0AA84');line(g,61,241,139,241,2,'#E8C39F');line(g,61,260,139,260,2,'#B68460');line(g,61,279,139,279,2,'#E8C39F');
    ellipse(g,83,272,24,14,THEME.wood,THEME.woodDark,3);line(g,75,270,96,264,4,THEME.woodDark);rect(g,59,268,8,7,THEME.wood);
    ellipse(g,99,248,6,6,THEME.malletHead,THEME.woodDark,2);line(g,103,245,133,226,6,THEME.mallet);
    ...
    outline(g,130,349,75,31,THEME.bed,THEME.outline2);rect(g,137,378,7,24,THEME.bedLeg);rect(g,191,378,7,24,THEME.bedLeg);
    ellipse(g,151,342,15,7,'#D9B56B',THEME.outline2,2);ellipse(g,96,366,8,8,'#F2C14E',THEME.outline2,2);line(g,96,359,96,373,3,'#5EA6E8');
