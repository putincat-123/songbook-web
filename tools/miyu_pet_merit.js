(() => {
  ...
  const ensure=()=>{const s=read();let changed=false;if(typeof s.merit!=='number'){s.merit=0;changed=true}if(!s.fortune||typeof s.fortune!=='object'){s.fortune={date:'',text:''};changed=true}if(!s.meritDaily||s.meritDaily.date!==today()){s.meritDaily={date:today(),incense:0,beads:0,pray:0};changed=true}if(changed)write(s);return s};
  const bubble=t=>{const b=document.getElementById('pxBubble');if(b&&b.textContent!==t)b.textContent=t};
  ...
  function refreshMeritLabel(){const s=ensure();const value=String(s.merit||0);document.querySelectorAll('[data-merit-count]').forEach(x=>{if(x.textContent!==value)x.textContent=value})}
  ...
  const petRoot=document.querySelector('.app');
  if(petRoot){
    const obs=new MutationObserver(mutations=>{
      if(mutations.some(m=>m.addedNodes&&m.addedNodes.length))scheduleInject();
    });
    obs.observe(petRoot,{subtree:true,childList:true});
  }
  ensure();inject();
})();
